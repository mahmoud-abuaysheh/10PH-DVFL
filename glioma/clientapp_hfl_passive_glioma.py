...
# Flower 1.26.1 — Passive silo HFL client for Glioma.
# Direct adaptation of clientapp_hfl_passive_diabetes.py.
#
# Architecture differences vs DIA:
#   BottomMLP:  in_dim -> 32 -> 16  (DIA: in_dim -> 16 -> 8)
#   ReconHead:  16 -> 32 -> in_dim  (DIA: 8 -> 16 -> in_dim)
#   out_dim = 16                    (DIA: 8)
#
# Partitioning: identical to DIA — IID np.array_split on shuffled train indices.
# NPZ default: glioma_aligned_vfl_hfl_cv.npz
...
# ── Models — must EXACTLY match Clientapp_vfl_glioma_router_offline.py ───────
# VFL client uses dropout=0.0 by default but the Dropout layer is still present
# in the Sequential, which shifts key indices. HFL must use the same definition.
class BottomMLP(nn.Module):
    """Glioma passive bottom encoder: in_dim -> 32 -> 16 (with Dropout placeholder)."""
...
class ReconHead(nn.Module):
    """Glioma passive reconstruction head: 16 -> 32 -> in_dim."""
...
def _init_if_needed(ctx):
    ...
    # Standardize with train-fold statistics
    mu  = X2[tr].mean(axis=0).astype(np.float32)
    sd  = (X2[tr].std(axis=0) + 1e-8).astype(np.float32)
    X2s = (X2 - mu) / sd

    # IID partition: shuffle train indices then split evenly (identical to DIA)
    ...
    st["dev"]     = dev
    st["ready"]   = True
    print(f"[Client] node_id={node_id} node_idx={node_idx} fold={fold} "
          f"partition={len(my_idx)} in_dim={st['in_dim']} K={K}")
...
def local_train(msg: Message, ctx: Context) -> Message:
    ...
    upd_b = {k: v.detach().cpu().numpy().astype(np.float32)
             for k, v in bottom.state_dict().items()}
    upd_r = {k: v.detach().cpu().numpy().astype(np.float32)
