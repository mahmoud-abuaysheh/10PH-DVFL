...
```bash
pip install -r requirements_diabetes.txt
pip install -e .
```
...
> **Note for HFL experiments:** In Flower 1.26.1, simulation settings
> (`num-supernodes`) are managed in `~/.flwr/config.toml`. Before running
> HFL pre-training, set the number of supernodes to match K:
>
> For K=10, edit `~/.flwr/config.toml`:
> ```toml
...
> ```
> For K=20, add an equivalent `[superlink.local-simulation-k20]` section
> with `options.num-supernodes = 20`.
...
for FOLD in 1 2 3 4 5; do
    NPZ_PATH=/path/to/diabetes_vfl_cv.npz \
    FOLD=$FOLD \
    PARTITION_JSON=/path/to/partitions_K10_train.json \
...
for FOLD in 1 2 3 4 5; do
    NPZ_PATH=/path/to/diabetes_vfl_cv.npz \
    FOLD=$FOLD \
    PARTITION_JSON=/path/to/partitions_K20_train.json \
    flwr run . local-simulation-k20
done
```

Replace `/path/to/` with the absolute path to the files on your machine.

After each fold completes, the HFL server saves:
- `pretrained_passive_bottom_hfl_fold{N}.pt` — the converged encoder checkpoint loaded by the Tier 2 VFL client
- `passive_aligned_embeddings_hfl_fold{N}.npz` — the silo-level embedding matrix generated by the HFL clients from their local data

> **Implementation note:** The Tier 2 VFL scripts for Condition 4 currently
> load the HFL checkpoint (`pretrained_passive_bottom_hfl_fold{N}.pt`) and
> run the encoder forward pass internally to generate passive silo embeddings.
> This is a simulation simplification where the VFL passive client holds the
> full aligned cohort data locally. In a real distributed deployment, the
> passive silo data would reside at the HFL clients and the pre-generated
> embedding file (`passive_aligned_embeddings_hfl_fold{N}.npz`) produced by
> the HFL server should be used directly in the Tier 2 VFL client instead,
> replacing the checkpoint loading step.

### Step 3 — Run Tier 2 VFL (Conditions 1, 2, 3, 4)

Update `pyproject.toml` to point to the correct scripts for your condition (see Experimental Conditions table). For Condition 1 (SplitNN), set `num-supernodes = 2` in `~/.flwr/config.toml`. For all decoupled conditions (2, 3, 4), also set `num-supernodes = 2`.
...
| `serverapp_hfl_passive_diabetes.py` | Flower HFL server for passive silo pre-training (Condition 4). After FedAvg training completes, the server collects embeddings from each HFL client and saves a silo-level embedding matrix alongside the checkpoint. |
...
| `requirements_diabetes.txt` | Python dependencies |
