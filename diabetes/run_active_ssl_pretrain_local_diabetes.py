# run_active_ssl_pretrain_local_diabetes.py
#
# Self-supervised pre-training for the active-silo encoder (X1) in the
# diabetes decoupled VFL experiment.
#
# This script implements the self-supervised Tier 1 pre-training stage for
# the active silo using a Denoising Autoencoder (DAE) objective. It is used
# when the active silo pre-training strategy is set to SSL rather than
...
# compact and robust representations of the active-silo features without
...
#
# This script trains locally on a single machine using only the active-silo
# feature matrix X1. No cross-silo communication occurs during this stage.
#
# Output:
#     pretrained_active_bottom_ssl_fold{fold}.pt
...
class BottomMLP_Paper(nn.Module):
    ...
    This architecture is shared across all pre-training modes (SSL and supervised)
    and must match the encoder architecture used in the client application.
    """
...
def load_npz(npz_path: str):
    ...
    Returns active-silo features X1, labels y (unused during SSL),
...
def main():
    ap = argparse.ArgumentParser(
        description="Self-supervised DAE pre-training for the active silo encoder (X1)."
    )
    ...
    ap.add_argument(
        "--out-dir", default="runs_active_ssl_diabetes",
    ...
    X1, _, folds, meta = load_npz(args.npz)
    ...
    mu = X1[tr].mean(axis=0, keepdims=True)
    sd = X1[tr].std(axis=0, keepdims=True) + 1e-8
    X1s = (X1 - mu) / sd

    dev = torch.device(args.device)
    X = torch.from_numpy(X1s).float().to(dev)
    ...
    for ep in range(1, args.epochs + 1):
        ...
        print(
            f"[Active SSL] fold={args.fold} epoch={ep}/{args.epochs} "
